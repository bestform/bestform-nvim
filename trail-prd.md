...
10. As a developer navigating to a file, I want to see that file highlighted in the trail, so that I know where I am in my exploration.
...
14. As a developer pressing 'q', I want the trail window to close, so that I can dismiss it when not needed.
...
- **Tested modules**: `session.lua` (state management), `tracker.lua` (edge detection and recording), `view.lua` (window rendering)
