...
assert_equal(session.record_file("/tmp/first.lua"), true, "active session records first file")
assert_equal(session.record_file("/tmp/second.lua"), true, "active session records second file")
assert_equal(session.record_file("/tmp/first.lua"), false, "duplicate file is ignored")
assert_list_equal(session.get_files(), { "/tmp/first.lua", "/tmp/second.lua" }, "files stay in first-visit order")
assert_equal(session.get_current_file(), "/tmp/first.lua", "duplicate entries still update current file")

session.stop()
assert_equal(session.is_active(), false, "stop deactivates session")
assert_equal(session.record_file("/tmp/after-stop.lua"), false, "stopped session does not record new files")
assert_list_equal(session.get_files(), { "/tmp/first.lua", "/tmp/second.lua" }, "stop preserves session data")
